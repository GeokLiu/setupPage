...
const SignUpStep2: React.FC = () => {
    ...
    // Pre-fill fields if IC data exists
    useEffect(() => {
        if (state?.icData) {
            setName(state.icData.fullName || "");
            setIcNumber(state.icData.icNumber || "");
            setAddress(state.icData.address || "");
        }
    }, [state]);

    const handleNext = () => {
        // You can pass this data to the next step if needed
    ...
    return (
        <Box textAlign="center" mt={5}>
            ...
            />
            <TextField
                label={translations[language].icNumber}
                value={icNumber}
